...
class PostgresCfModuleBuilder extends BuildTaskHandlerInternal {
  ...
  init() {
    ...
    this.task.options.compileDest = path_1.default.resolve(
      this.task.dest,
  ...
  async _writePackageJson() {
    ...
    if (this.isStagingBuild() && !exists) {
      ...
      if (
        (_b = (_a = rootPackageJson.cds) === null || _a === void 0 ? void 0 : _a.migrations) === null || _b === void 0
          ? void 0
          : _b.schema
      ) {
  ...
  async _writeUndeployJson(src) {
    ...
    if (
      migrationOptions.deploy.undeployFile &&
      (0, fs_1.existsSync)(path_1.default.join(src, migrationOptions.deploy.undeployFile))
    ) {
      this.logger.log(`[cds] - ${this.task.for}: copy existing undeploy.json`)
      await this.copy(path_1.default.join(src, migrationOptions.deploy.undeployFile)).to(
        path_1.default.join(this.task.dest, 'undeploy.json')
      )
    }
  }
  /**
  ...
  async _writeManifestYml() {
    ...
    if (
      (await fs.pathExists(path_1.default.join(this.task.src, FILE_NAME_MANIFEST_YML))) ||
    ...
    try {
      const descriptor = await getHanaDbModuleDescriptor(
        ...
        path_1.default.basename(this.task.src),
        this.logger
      )
